    ...
    async def extract(self, source_text: str) -> GenerationKeys:
        return await self.ai_client.parse(
            system_prompt=(
                ...
                "Не придумывай отсутствующие требования. Известные параметры раскладывай "
                "по полям схемы. Любые другие пользовательские ключи, которые могут влиять "
                "на генерацию или проверку, сохраняй в custom_parameters как name/value."
            ),
